package com.example.artificialintelligenceforlegalassistance.screens.home.family.divorce
...
@Composable
fun familyLaws(navController: NavController, paddingValues: PaddingValues) {
    val chatViewModel: ChatViewModel = viewModel()
    val chatState by chatViewModel.chatState.collectAsState()

    // Launch effect to send initial prompt
    LaunchedEffect(Unit) {
        chatViewModel.onEvent(ChatUiEvent.SendPrompt("provide  5 summarized case documents from supreme court of India regarding \"Divorce\" with year and atleast 250 words each give case title, judge names, case description"))
    }
